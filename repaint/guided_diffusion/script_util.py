...
def model_and_diffusion_defaults():  # 返回网络相关的默认参数字典
    """
...
def create_model_and_diffusion(  # 创建model和diffusion
        image_size,
        class_cond,
        learn_sigma,
        num_channels,
    ...
    diffusion = create_gaussian_diffusion(  # 创建diffusion框架
...
def create_model(  # 创建UNet
    ...
    return UNetModel(
        ...
        out_channels=(conf.model.out_channels if not learn_sigma else conf.model.out_channels * 2),
...
def sr_create_model(
    ...
    return SuperResModel(
        ...
        in_channels=conf.model.in_channels,
        condition_channels=conf.model.condition_channels,
        model_channels=num_channels,
        out_channels=(conf.model.out_channels if not learn_sigma else conf.model.out_channels * 2),
        num_res_blocks=num_res_blocks,
        attention_resolutions=tuple(attention_ds),
...
def create_gaussian_diffusion(  # 创建一个spaceDiffusion
        *,
        steps=1000,
        learn_sigma=False,
        sigma_small=False,
    ...
    if hasattr(conf, 'use_value_logger'):
