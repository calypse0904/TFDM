...
class Default_Conf(NoneDict):
    ...
    def get_dataloader(self, dset='train', dsName=None, batch_size=None, return_dataset=False):
        ...
        candidates = self['data'][dset]#使用dset指定配置文件中，data下的哪种数据模式，现有配置文件只给了eval模式
        ds_conf = candidates[dsName].copy()#获取当前模式下所有的配置信息

        mask_loader_flag=ds_conf.get('mask_loader', False)
        from guided_diffusion.image_datasets import load_data_inpa
        return load_data_inpa(**ds_conf, conf=self,mask_loader_flag=mask_loader_flag)
    ...
    def eval_imswrite(self, srs=None, img_names=None, dset=None, name=None, ext='tif', lrs=None, gts=None, gt_keep_masks=None, verify_same=True):
